...
public class RegisterCustomerExecutor implements InstructionExecutor {
    ...
    public void visitDatabase() throws SQLException {
        ...
        String sql = "INSERT INTO CUSTOMER (PHONE_NUMBER, USER_NAME, PASSWORD) VALUES ('" + this.phoneNumber + "', '" + this.userName + "', '" + this.password +  "')";
        try {
            db.insert(sql);
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public static boolean isValidPassword(String password) {
        if (!noQuotationMark(password)) {
            System.out.println("The password can not contain any quotation mark");
            return false;
        }
    ...
    public static boolean isRepeatedPhoneNumber(String phoneNumber) throws SQLException {
        Database db = Database.getDataBase();
        return db.contains("CUSTOMER", "PHONE_NUMBER", phoneNumber);
    }
