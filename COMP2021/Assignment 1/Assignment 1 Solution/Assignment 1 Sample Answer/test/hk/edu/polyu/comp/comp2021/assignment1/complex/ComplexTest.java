...
public class ComplexTest {
    ...
    @Test
    public void testSubstraction_02() {
        Complex c1 = new Complex(new Rational(0, 1), new Rational(0, 1));
        Complex c2 = new Complex(new Rational(1, 3), new Rational(1, 4));

        Complex cSum = c1.subtract(c2);
        cSum.simplify();

        assertEquals("(-1/3,-1/4)", cSum.toString());
    }
    ...
    public void testMultiplication_02(){
        Complex c1 = new Complex(new Rational(1, 1), new Rational(0, 1));
        Complex c2 = new Complex(new Rational(1, 5), new Rational(5, 8));

        Complex cMul = c1.multiply(c2);
        cMul.simplify();

        assertEquals("(1/5,5/8)", cMul.toString());
    }

    @Test
    public void testMultiplication_03(){
    ...
    @Test
    public void testDivision_02() {
        Complex c1 = new Complex(new Rational(1, 3), new Rational(1, 3));
        Complex c2 = new Complex(new Rational(1, 4), new Rational(1, 4));

        Complex cDiv = c1.divide(c2);
        cDiv.simplify();

        assertEquals("(4/3,0/1)", cDiv.toString());
    }
}
